...
odoo.define('ks_dashboard_ninja.ks_dashboard', function (require) {
    ...
    var KsDashboardNinja = AbstractAction.extend( {
        ...
        espacement_chiffre: function(nombre){
            // nombre test
        ...
        //    b = 0, // espacement
        //    i = 0; // boucle
        var chaine = String(nombre) // convertit en chaine
        var newChaine = '' // nouvelle chaine vide
        var longueur = chaine.length // longueur du nombre
        var b = 0 // espacement
        var i = 0 // boucle
       ...
       // return résultat
        ...
        ksNumFormatter: function (num, digits) {
            ...
            },
            {
                value: 1E15,
                // symbol: "P"
                symbol: "1000000000000000"
            },
            {
                value: 1E18,
                // symbol: "E"
                symbol: "1000000000000000000"
            }
                // {
                //     value: 1E3,
                //     symbol: "k"
            ...
            // if (negative) {
            //     return "-" + ((num / si[i].value)* parseInt(si[i].symbol));
            // } else {
            //     return (num / si[i].value) * parseInt(si[i].symbol);
            // }
            // if(negative){
            //     return "-" +((num / si[i].value) * parseInt(si[i].symbol)).toFixed(digits);
            // }else{
            //     return ((num / si[i].value) * parseInt(si[i].symbol)).toFixed(digits); 
            // }
        //     esorina lo
            if(negative){
                return "-" +this.espacement_chiffre(((num / si[i].value) * parseInt(si[i].symbol)).toFixed(digits));
            }else{
                return this.espacement_chiffre(((num / si[i].value) * parseInt(si[i].symbol)).toFixed(digits)); //io 
            }
        // },
        },
        ...
        _ksRenderDashboardTile: function (tile, ks_container_class, ks_inner_container_class) {
            ...
            switch (tile.ks_layout) {
                case 'layout1':
                    ...
                    break;
                    // hitao io oo, 1 virgule 5, dia le eo ambony 200 000 mihintsy ,izay hoe mila atao multiplicatio ni e mihintsy
                    // ie ka io ne le namparako e, andao alo ary jerene aho aiza ireo valeur ireook
            ...
            if (!kpi_data[1]) {
                ...
                switch (field.ks_data_comparison) {
                    case "None":
                        ...
                        var count = parseInt(self.ksNumFormatter(count_1,1)) / parseInt(self.ksNumFormatter(count_2,1));
                        item_info['count'] = String(count);
